...
- `/api/bookmarks` (GET) — List bookmarked documents for a user (default: "default"). Returns enriched entries with document metadata (title, source, file_path, dates, size). Accepts optional `?user_id=` query param.
- `/api/bookmarks` (POST) — Add a bookmark. Body: `{doc_id, user_id?}`. Idempotent — re-adding keeps the original timestamp.
- `/api/bookmarks/check` (POST) — Bulk-check bookmark status. Body: `{doc_ids: [...], user_id?}`. Returns `{doc_id: boolean}` map.
- `/api/bookmarks/:doc_id` (DELETE) — Remove a bookmark. Accepts optional `?user_id=` query param.
